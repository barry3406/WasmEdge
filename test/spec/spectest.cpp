...
void SpecTest::executeCommands(Span<const Wast::ScriptCommand> Commands,
  ...
  for (const auto &Cmd : Commands) {
    ...
    case Wast::CommandType::Module: {
      ...
      bool SkipValidation = IsComponent && !checkComponentSupported(
                                               UnitName, WasmPhase::Validation);
    ...
    case Wast::CommandType::Thread: {
      ...
      auto AliasCopy = Alias;

      ThreadMap.emplace(
          ThreadName,
          std::thread([this, ChildCtx, SubCmds = Cmd.SubCommands, Conf,
                       IsComponent, TestFile = std::string(TestFile),
                       AliasCopy = std::move(AliasCopy)]() mutable {
